...
in {
  options = {
    music = {
      ...
      ncmpcpp.enable = lib.mkEnableOption "enables ncmpcpp";
      importMusic.enable = lib.mkEnableOption "enables importMusic";
  };
  ...
  config = lib.mkIf config.music.enable (lib.mkMerge [
    (lib.mkIf config.music.mpd.enable {
      services.mpd = {
        ...
        playlistDirectory = "${services.mpd.musicDirectory}"/.playlists; # TODO: Use user directory dynamically
