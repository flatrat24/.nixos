  ...
  config = lib.mkIf config.neovim.plugins.telescope.enable (lib.mkMerge [
    {
      programs.nixvim = {
        ...
        keymaps = [
          ...
          { mode = [ "n" ]; key = "<leader>fc"; action = "<cmd>Telescope command_history<CR>"; options = { noremap = true; silent = true; desc = "Command history"; }; }
          { mode = [ "n" ]; key = "<leader>fh"; action = "<cmd>Telescope help_tags<CR>"; options = { noremap = true; silent = true; desc = "Help"; }; }
        ];
      };
    }
  ]);
